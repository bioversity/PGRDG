...
$.last_activity = function(full) {
	...
	if($.storage_exists("pgrdg_user_cache.user_activity")) {
		l = storage.get("pgrdg_user_cache.user_activity");
		l.reverse();

		if(full) {
			last_activity = $.ucfirst(l[0].action) + "<br />on " + l[0].date;
		} else {
			last_activity = l[0].date;
		}
	} else {
		if(full) {
