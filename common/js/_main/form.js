			...
			$.each(id_arr, function(idk, idv) {
				...
				*/
				...
				switch(forms.size) {
					...
					case "single":
					default:
						var form_size = "col-lg-4 col-md-6 col-sm-12";
						break;
				}
				...
				var help = '<small class="help-block" style="color: #999; margin-bottom: -3px; font-size: 13px;"><br />' + $.get_title(forms) + '</small>',
...
$.fn.addTraitAutocomplete = function(options, data, callback) {
	...
	remoteAutocomplete = new Bloodhound({
		...
		remote: {
			...
			//BACK url: "API/?type=service&proxy=" + "%QUERY",
			replace: function(url, query) {
				...
				//var state = "true&address=" + $.utf8_to_b64("{SERVICE_URL}?" + kAPI_REQUEST_OPERATION + "=" + kAPI_OP_MATCH_TAG_LABELS + "&" + kAPI_REQUEST_LANGUAGE + "=" + lang + "&" + kAPI_REQUEST_PARAMETERS + "=" + $.rawurlencode('{"' + kAPI_PAGING_LIMIT + '":50,"' + kAPI_PARAM_REF_COUNT + '": "' + kAPI_PARAM_COLLECTION_UNIT + '","' + kAPI_PARAM_PATTERN + '":"'  + $("#" + options.id).val() + '","' + kAPI_PARAM_OPERATOR + '": ["$' + $("#" + options.id + "_operator").attr("class") + '"' + ($("#main_search_operator_i").is(":checked") ? ',"$i"' : "") + ']}'));
			//BACK console.log("true&address=" + "{SERVICE_URL}?" + kAPI_REQUEST_OPERATION + "=" + kAPI_OP_MATCH_TAG_LABELS + "&" + kAPI_REQUEST_LANGUAGE + "=" + lang + "&" + kAPI_REQUEST_PARAMETERS + "=" + $.rawurlencode('{"' + kAPI_PAGING_LIMIT + '":50,"' + kAPI_PARAM_REF_COUNT + '":"' + kAPI_PARAM_COLLECTION_UNIT + '","' + kAPI_PARAM_PATTERN + '":"'  + $("#" + options.id).val() + '","' + kAPI_PARAM_OPERATOR + '": ["$' + $("#" + options.id + "_operator").attr("class") + '"' + ($("#main_search_operator_i").is(":checked") ? ',"$i"' : "") + ']}'));	
		...
		switch(jQuery.type(value)) {
			case "object":
				if(jQuery.isArray(value)) {
					...
					content += '<li class="list-group-item"><span class="fa fa-caret-right fa-fw"></span><a href="javascript:void(0);" data-toggle="collapse" data-target="#' + $.md5(label + id) + '">' + tag_label + '</a><div id="' + $.md5(label + id) + '" class="collapse"><ul class="list-group">' + $.cycle_row_data({res: options.res, row_obj: value}) + "</ul></div></li>";
				}
				break;
			case "array":
				switch(tag_type) {
					...
					case kTYPE_STRUCT:
						if($.obj_len(value) == 1) {
							$.each(value, function(key, val) {
								if($.obj_len(val) > 1) {
									content += '<li class="list-group-item"><span class="fa fa-caret-right fa-fw"></span><a href="javascript:void(0);" data-toggle="collapse" data-target="#' + $.md5(label + id) + '">' + tag_label + '</a><div id="' + $.md5(label + id) + '" class="collapse"><ul class="list-group">' + $.cycle_row_data({res: options.res, row_obj: val}) + "</ul></div></li>";
								} else {
							...
							content += '<li class="list-group-item"><span class="fa fa-caret-right fa-fw"></span><a href="javascript:void(0);" data-toggle="collapse" data-target="#' + $.md5(label + id) + '">' + tag_label + '</a><div id="' + $.md5(label + id) + '" class="collapse"><ul class="list-group">';
							$.each(value, function(key, val) {
								$.each(val, function(k, v) {
									contents += '<ul class="list-group">';
									if($.type(v) == "array") {
										var arr_content = "", sub_array_label = [];
										for(var i = 0; i < v.length; i++) {
											sub_array_label.push($.get_enum_label_from_string(options.res, v[i]));
										}
										arr_content += '<li class="list-group-item"><b>' + $.get_tag_label_from_string(options.res, k) + "</b><ul><li>: " + sub_array_label.join("</li><li>") + "</li></ul></li>";
										content += arr_content;
									} else {
										content += '<li class="list-group-item"><b>' + $.get_tag_label_from_string(options.res, k) + "</b>: " + $.get_enum_label_from_string(options.res, v) + "</li>";
									}
									contents += '</ul>';
								});
							});
...
$.get_enum_label_from_string = function(res, string) {
	...
	if(!jQuery.isEmptyObject(res)) {
		$.each(res[kAPI_RESPONSE_RESULTS][kAPI_PARAM_COLLECTION_TAG], function(tag, data_tag) {
			if (string == tag) {
				find++;
			}
		});
		if(find > 0) {
			...
			$.each(res[kAPI_RESPONSE_RESULTS][kAPI_PARAM_COLLECTION_TERM], function(term, data_term) {
				if (string == term) {
					find++;
				}
			});
			if(find > 0) {
				label = res[kAPI_RESPONSE_RESULTS][kAPI_PARAM_COLLECTION_TERM][string][kTAG_LABEL];
			} else {
				label = string;
			}
		}
...
$.add_input = function(options) {
	...
	$.each(operators, function(k, v) {
		if(!v.main) {
			if(v.label !== undefined) {
				checkbox += '<div class="checkbox"><label title="' + ((v.title !== undefined) ? v.title : "") + '" onclick="$(this).find(\'input\').is(\':checked\') ? $(this).find(\'input\').val(\'$i\') : $(this).find(\'input\').val(\'\');"><input type="checkbox" id="' + options.id[0] + '_operator_' + v.key.replace("$", "") + '" ' + ((v.selected) ? 'checked="checked"' : "") + ' title="' + ((v.title !== undefined) ? v.title : "") + '" name="case_sensitive" value="$i" /> ' + v.label + '</label></div>';				
			}
		} else {
			checkbox = "";
			if(v.main !== undefined) {
