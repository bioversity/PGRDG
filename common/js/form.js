		...
		generate_form = function(single_form) {
			...
			$.each(cluster, function(term, id_arr) {
				$.each(id_arr, function(idk, idv) {
					...
					var enable_disable_btn = '<a href="javascript:void(0);" onclick="$.toggle_form_item($(this), \'' + idv + '\');" class="pull-left" title="' + i18n[lang].messages.enable_item + '"><span class="fa fa-square-o"></span></a>',
					badge = '<span class="badge pull-right">' + ((forms.count !== undefined && forms.count > 0) ? forms.count : 0) + '</span>',
					mask = ((forms.count !== undefined && forms.count > 0) ? '<div id="' + $.md5(idv) + '" onclick="$.toggle_form_item($(\'#' + $.md5(idv) + '\'), \'' + idv + '\');" class="panel-mask"><span class="fa fa-check"></span><small>' + i18n[lang].interface.btns.activate + '</small></div>' : '<div class="panel-mask unselectable"></div>');
		...
		remoteAutocomplete = new Bloodhound({
			...
			remote: {
				...
				replace: function(url, query) {
					...
					query_obj[kAPI_PARAM_OPERATOR] = [$("#main_search_operator").attr("class") + '"' + ($("#main_search_operator_i").is(":checked") ? ',"$i"' : "")];
		...
		remoteAutocomplete = new Bloodhound({
			...
			remote: {
				...
				replace: function(url, query) {
					...
					var state = "true&query=" + $.utf8_to_b64(config.service.url + "Service.php?" + kAPI_REQUEST_OPERATION + "=" + options.operator + "&" + kAPI_REQUEST_LANGUAGE + "=" + lang + "&" + kAPI_REQUEST_PARAMETERS + "=" + $.rawurlencode('{"' + kAPI_PARAM_LOG_REQUEST + '":true,"' + kAPI_PAGING_LIMIT + '":50,"' + kAPI_PARAM_REF_COUNT + '":"' + kAPI_PARAM_COLLECTION_UNIT + '","' + kAPI_PARAM_PATTERN + '":"' + $("#" + options.id + "_input").val() + '","' + kAPI_PARAM_EXCLUDED_TAGS + '":[' + exclude + '],"' + kAPI_PARAM_OPERATOR + '": ["$' + $("#" + options.id + "_operator").attr("class") + '"' + ($("#filter_search_summary_operator_i").is(":checked") ? ',"$i"' : "") + ']}'));
		...
		$("#" + options.id + " .typeahead").typeahead({
			...
			if($("#" + options.id + "_input").val().length > 3) {
				...
				$("#autocomplete_undo_btn").on("click", function() {
					apprise(i18n[lang].messages.grouping.are_you_sure_to_remove, {confirm: true}, function(r) {
						if(r) {
		...
		*/
		...
		$.generate_summaries = function(options, colour, callback) {
			...
			$.each(options[kAPI_RESPONSE_RESULTS], function(domain, values) {
				...
				result_description_span_right.append('<a class="btn text-info transparent" href="javascript: void(0);" onclick="$.show_raw_data(\'' + storage_id + '\', \'' + domain + '\', \'0\', \'50\', \'' + $.rawurlencode(h) + '\')\"><span class="fa fa-list-alt"></span>' + i18n[lang].interface.btns.view_data + '</a>');
				if(values.points > 0) {
					result_description_span_right.append(' <span class="hidden-xs hidden-sm text-muted">|</span><a class="btn ' + ((values.points > 10000) ? "text-warning" : "") + ' transparent" href="javascript: void(0);" onclick="$.show_data_on_map(\'' + storage_id + '\', \'' + domain + '\', \'' + $.rawurlencode(h) + '\')" title="' + values.points + ' nodes for this entry"><span class="ionicons ion-map"></span>' + i18n[lang].interface.btns.view_map + ' <sup class="text-muted">' + values.points + '</sup></a>');
				}
			...
			*/
			$.fn.add_remove_item_to_stage = function(storage_id, id, tag, name, contains, info, display_message) {
				...
				if($(this).closest("ul:not(.level2) > li").find("a").hasClass("btn-default")) {
					...
					if(display_message) {
						$("#modal_messages").removeClass("alert-danger").addClass("alert-success").find("div.text-left").text(i18n[lang].messages.added.replace("{X}", '"' + name + '"'));
						$("#modal_messages").show().delay(1500).fadeOut(300);
					}
				}
				if(!multiple) {
