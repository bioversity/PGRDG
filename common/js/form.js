...
$.fn.execute_search = function(offsets) {
	var $item = $(this),
	active_forms = {},
	af_obj = {},
	rt = {};
	...
	if(!$item.hasClass("disabled")) {
		...
		$.toggle_form_item($("#" + $.md5(af_obj.tags)), af_obj.tags);
	}
};
...
$.build_big_buttons_menu = function() {
	...
	$.iterate_buttons = function(v, root) {
		...
		$.each(v.links, function(kk, vv) {
			var $li = $('<li>'),
			$link_icon = $('<span class="fa fa-fw">'),
			$list_icon = $('<span class="fa fa-fw fa-caret-right">');
			if(vv.link !== undefined && vv.link !== null && v.link !== "") {
				$sublink = $('<a>').attr({
					"href": vv.link,
			...
			if($.obj_len(vv.childs) > 0) {
				var $sublink = $('<a>').attr({
					"href": "javascript:void(0);",
					"onclick": "$(this).collapse_submenu();",
					"class": "btn-link"
				}).text(vv.name),
				$ul2 = $('<ul class="fa-ul" style="display: none;">');

				$li.append($list_icon);
				$li.append($sublink);
				$.each(vv.childs, function(kkk, vvv) {
					$li2 = $('<li>'),
					$searchlink = $('<a>').attr({
						"href": "javascript:void(0);",
						"onclick": "$.show_summary_directly('" + JSON.stringify(vvv.criteria) + "')",
						"class": "text-info"
					}).text(vvv.name);

					$li2.append($searchlink);
					$ul2.append($li2);
				});
				$li.append($ul2);
			}
		...
		success: function(response) {
			...
			$.each(response.buttons, function(k, v) {
				if(v.icon !== "") {
					...
					$link = $('<a>').attr({
						"href": "javascript: void(0);",
						"class": (($.obj_len(v.links) === 0) ? "disabled" : "")
					}),
